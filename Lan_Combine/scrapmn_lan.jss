...
My_tempdir:=fn +'\JSS\';
fncustomlan := trim(AskFnForOpen('Select Custom language JoPPS database :', My_tempdir, 'scrapmn.db', 'db','DataBases (*.db)|*.db'));
lenfncustom:=strlen(fncustomlan);
fncustom := strleft(fncustomlan,lenfncustom-10);
fns:=fn+'\LAN\';
db_newlan := dbtable.create();
db_newlan.DataBaseName := fns;
db_newlan.tableName := 'scrapmn.db';
...
db_customlan.Sql:= 'SELECT FORM_ID,CTRL_ID,LAN_USER,HIN_USER FROM scrapmn.db where FORM_ID='+S_IDFORM+' and CTRL_ID='+S_IDCTRL+"'";
